...
- **안전 실행 제한:**
  - 최대 실행 step: **500 step** (tracer 내 `StepLimitExceeded` 예외로 처리)
  - 실행 timeout: **5초** — 메인 스레드 `setTimeout`으로 관리. 5초 경과 시 아래 토스트 UX 흐름 실행.
    ...
    **timeout 토스트 UX 흐름:**
    ...
    const timeoutId = setTimeout(() => {
      worker.terminate()
      showToast('⚠️ 실행 시간이 너무 길어 안전을 위해 실행을 중단했습니다. 환경을 재설정합니다.')
...
- **varTypes (Worker 전용):** `trace_log` 수집 직후 Worker에서 `extract_var_types_union(trace_log)` 실행 → `postMessage`에 `varTypes` 동봉. 메인 스레드는 `rawTrace`를 다시 돌지 않음 (500 step 등에서 UI 미세 프리징 방지).
...
def tracer(frame, event, arg):
    ...
    if step_count >= MAX_STEPS:
        raise StepLimitExceeded("Step limit exceeded (Max 500 steps)")
...
Phase 2 — 설명 /api/explain (청크 단위 순차 호출)
  ...
  → 마지막 chunk에 runtimeError 있으면 aiError(root_cause + fix_hint) 추가
...
출력 규칙:
  ...
  - 에러 step이 있으면 반드시 "aiError" 키 아래에 root_cause와 fix_hint를 담을 것
...
- **Break & Rewind:** 에러 발생 시점(Break)에서 실행 중단. 에러 이전 시점으로 자유롭게 복기(Rewind) 가능.
  ...
  "rawTrace": [
    {
      ...
      "vars": {
        ...
        "visited": {"type": "list2d", "value": [[true, false], [false, false]]},
      ...
      "parent_frames": {
        "0": {
          "grid": {"type": "list2d", "value": [[0,1],[1,0]]}
        }
      }
    }
  ],
...
const normalized = Array.from({ length: rawChunk.length }, (_, i) =>
  annotatedChunk[i] ?? EMPTY_ANNOTATED
)
...
```json
{
  ...
  "aiError": {
    ...
    "fix_hint": "탐색 전 행/열 범위 조건(0 <= r < rows)을 확인하세요."
  }
}
```
...
├───────────────────────────────────────────────────────────┤
│  Bottom (Timeline): 루프/분기 점(Dot) 표시. 에러 지점 빨간 X │
└───────────────────────────────────────────────────────────┘
```
...
| step 에러 발생 | 자동 Break, 타임라인 빨간 X, Right Pane에 `runtimeError` + `aiError` 표시 |
...
| HYBRID 멀티패널 | 🔲 | 시간 여유 시 추가 |
| Break & Rewind | ✅ | |
| Auto-play + 속도 조절 | ✅ | |
| 타임라인 (에러 앵커 포함) | ✅ | AST branchLines로 루프/분기 Dot 표시 |
| 변수 모니터링 패널 | ✅ | |
...
2. 의도적으로 인덱스 에러 있는 코드 → 에러 step에서 Break + AI의 root_cause + fix_hint 표시
...
1. **GRAPH / HYBRID 전략 완성:** Dijkstra, 트리 재구성 등 지원.
2. **Edge Case 생성기:** AI가 사용자의 코드를 터뜨릴 수 있는 극한의 입력값(Test Case)을 자동 생성.
3. **협업 디버깅:** 특정 디버깅 세션(step과 시각화 상태 포함)을 고유 URL로 공유.
4. **코드 최적화 제안:** 시간/메모리 초과 감지 시 AI가 더 효율적인 알고리즘으로의 리팩토링 가이드 제공.
5. **JS/Java 지원:** 각 언어의 브라우저 실행 환경(QuickJS 등) 도입.
6. **대형 trace 전송 최적화 (성능 병목 시):** Worker 내부에서 step 간 **Delta 방식**으로 변경분만 기록하거나, `parent_frames` 전체 복사 대신 참조·공유 구조로 줄여 `postMessage` 페이로드와 직렬화 비용을 절감. (MVP는 전량 스냅샷 + `serialize` 깊이 제한으로 충분; 프로파일링 후 도입)
7. **Call Stack 미니 트리 비주얼:** `scope.depth`·함수명을 활용해 상단에 얕은 호출 스택 트리(또는 단계형 인디케이터)를 표시. 재귀/DFS 학습 시 "현재 몇 번째 호출인지"를 Breadcrumb 텍스트만으로 보는 것보다 직관적으로 전달.
