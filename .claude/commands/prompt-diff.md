...
2. **계약 조건 정적 검증** (프롬프트 텍스트 분석):
   ...
   **/api/explain 계약:**
   - "출력 배열 길이 === 입력 steps 길이" 1:1 대응 지시가 있는지
   - 에러 분석 키가 `aiError`로 고정되어 있는지 (다른 키 이름 금지 명시)
   - `aiError: null` 명시 지시가 있는지
   - `visual_actions`에 색상/스타일 포함 금지 지시가 있는지
...
- [ ] explain: 출력 길이 === 입력 길이 지시됨
- [ ] explain: aiError 키 이름 고정 + null 명시 지시됨
- [ ] explain: visual_actions 색상/스타일 금지 지시됨
